...
//					$.pdialog.switchDialog(dialog);
		...
		close:function(dialog) {
			/* update by ymx 2014/10/21 */
			var dialogID = $(dialog).data("id");
			if(typeof dialogID == "undefined"){
				dialogID = dialog;
			}
			var closeType = $("#closeType").val(); //取得关闭类型，默认为close
			var isShowConfirm = $(dialog).data("isShowConfirm"); //是否显示关闭确认
			if(typeof isShowConfirm == "undefined"){
				isShowConfirm = true;
			}
			if(typeof dialogID == "string" && dialogID.indexOf("exam_")!=-1 
				 && typeof closeType != "undefined" && closeType == "close" && isShowConfirm){
				//考试对话框
				alertMsg.confirm("若未保存答案，关闭后将丢失当前答案！确定要关闭吗？", {
					okCall: function(){
						/*var examForm = $("#examForm");
						var action_val = examForm.attr("action");
						examForm.attr("action", action_val+"&type=hand");
						examForm.submit();*/
		...
		/* update by ymx 2014/11/11 */
		/* closeCurrent:function(){ //原来的function
			this.close($.pdialog._current);
		},*/
		closeCurrent:function(isShowConfirm){ //修改后的function
			var dialog = $.pdialog._current;
			dialog.data("isShowConfirm", isShowConfirm);
			this.close(dialog);
		},
